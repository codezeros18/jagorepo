...
// ── Types (mirrors frontend UploadScanResponse shape) ─────────────────────────
...
export function generatePdfReport(data: ReportData, output: Writable): PDFKit.PDFDocument {
  ...
  // ── Per-finding detail (vulnerable only) ─────────────────────────────────
    ...
    for (const f of vulnFindings) {
      ...
      if (heuristicReasons.length > 0) {
        ...
        doc.font("Helvetica-Bold").fontSize(8.5).fillColor("#92400E")
          .text("Signal Heuristik:", MARGIN + 8);
        doc.moveDown(0.1);
        ...
        doc.font("Helvetica").fontSize(7.5).fillColor(COLOR.muted)
          .text(
            "Catatan: signal heuristik adalah indikasi awal, bukan bukti malware.",
            MARGIN + 14, doc.y, { width: CONTENT_W - 22 }
          );
        doc.moveDown(0.2);
      }
      ...
      doc.font("Helvetica-Bold").fontSize(8).fillColor(COLOR.primary)
        .text("Rekomendasi:", MARGIN + 16, recY + 6);
